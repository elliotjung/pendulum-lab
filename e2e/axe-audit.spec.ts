...
/**
 * Full-page axe-core audits. Contract: no critical/serious/moderate WCAG 2.0/2.1 A/AA
...
test('axe: default lab surface has no moderate-or-higher violations', async ({ page }) => {
  await page.goto('/');
...
test('axe: validation tab has no moderate-or-higher violations', async ({ page }) => {
...
test('axe: research workspace has no moderate-or-higher violations', async ({ page }) => {
...
test('axe: open trust drawer has no moderate-or-higher violations', async ({ page }) => {
  await page.goto('/');
  await page.waitForFunction(() => Boolean((window as unknown as { __modernShell?: unknown }).__modernShell));
  await page.waitForTimeout(1200);
