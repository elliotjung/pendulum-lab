...
test('Research+ tab runs the SDE ensemble, parameter fit, and PCE surrogate', async ({ page }) => {
  ...
  await page.waitForFunction(
    () => (document.getElementById('rpSdeStatus')?.textContent ?? '').includes('realisations'),
    undefined,
    { timeout: 30000 }
  );
  ...
  await page.waitForFunction(
    () => (document.getElementById('rpFitStatus')?.textContent ?? '').includes('ĝ'),
    undefined,
    { timeout: 30000 }
  );
  ...
  await page.waitForFunction(
    () => (document.getElementById('rpPceStatus')?.textContent ?? '').includes('R²'),
    undefined,
    { timeout: 30000 }
  );
