...
test('main trajectory OffscreenCanvas opt-in renders or safely retains Canvas2D', async ({ page, browserName }) => {
  ...
  await page.waitForFunction(
    (time) => (window as unknown as { __modernLab: { diagnostics(): { time: number } } }).__modernLab.diagnostics().time > time,
    first.time
  );
...
test('cinematic WebGL2 trail opt-in executes the batched backend in Chromium', async ({ page, browserName }) => {
  test.skip(browserName !== 'chromium', 'WebGL2 promotion probe is Chromium-specific; other engines retain Canvas2D fallback.');
