...
function jacobianProvenanceForSpec(spec: SystemSpec): JacobianProvenance | undefined {
  switch (spec.kind) {
    case 'double':
    case 'double-string':
    case 'driven':
...
/**
 * Machine-readable tangent-source contract for a model spec.  Consumers that
 * report Lyapunov/FTLE estimates can display this instead of silently treating
 * a central-difference fallback as equivalent to an exact model tangent.
 */
export function jacobianTrustForSpec(spec: SystemSpec): JacobianTrustMetadata {
  const provenance = jacobianProvenanceForSpec(spec);
  if (!provenance) {
    return {
  ...
  return {
    ...
    caveat:
      provenance === 'automatic-differentiation'
        ? 'Tangent dynamics are generated by forward-mode automatic differentiation of the implemented mass-matrix model and are regression-checked against finite differences.'
        : 'Tangent dynamics use a closed-form analytic Jacobian of the implemented model and are regression-checked against finite differences.'
  };
}

function withSpecJacobian(spec: SystemSpec, derivative: Derivative): Derivative {
  const jacobian = buildJacobian(spec);
  const provenance = jacobianProvenanceForSpec(spec);
  if (jacobian && provenance) {
    derivative.jacobian = jacobian;
    derivative.jacobianProvenance = provenance;
  }
  return derivative;
}
