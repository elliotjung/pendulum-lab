...
import type { ExperimentWorkflowStep, TrajectoryStage } from './experimentWorkflow';
import type { ExperimentGoal } from './experimentRecipes';
...
export type SharedAngleUnit = 'rad' | 'deg';
...
// V4 makes the complete nearby-trajectory rule and guided-workflow position
// first-class. Epsilon is canonical SI (rad for theta, rad/s for omega).
// prettier-ignore
export interface SharedExperimentV4 extends Omit<SharedExperimentV3, 'v' | 'execution'> { v: 4; execution: Omit<SharedExperimentV3['execution'], 'ensemble'> & { ensemble: { count: number; epsilon: number; variable: PerturbationVariable; pattern: PerturbationPattern; seed: number } | null }; preferences: { angleUnit: SharedAngleUnit }; workflow: { goal: ExperimentGoal; step: ExperimentWorkflowStep; trajectoryStage: TrajectoryStage } }
export type SharedExperimentPayload = SharedExperimentV1 | SharedExperimentV2 | SharedExperimentV3 | SharedExperimentV4;
// prettier-ignore
export interface SharedExperimentDiagnostic { severity: 'info' | 'warning' | 'error'; code: string; message: string; fields?: string[] }
// prettier-ignore
export interface SharedExperimentDecodeResult { ok: boolean; payload: SharedExperimentV4 | null; diagnostics: SharedExperimentDiagnostic[] }
// prettier-ignore
