...
export class BasinTab extends TabController {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      this.dom.setText(
        'basinSbb',
        `${result.boundaryBasinEntropy.toFixed(4)} ± ${result.boundaryBasinEntropyStdError.toFixed(4)}`
      );
      this.dom.setText(
        'basinDim',
        `${result.boxCountingDimension.toFixed(3)} ± ${result.boxCountingStdError.toFixed(3)}`
      );
      this.dom.setText(
        'basinFractal',
        result.fractalBoundary ? 'Sbb > ln2 ✓' : `dim≈${result.boxCountingDimension.toFixed(2)}`
      );
      this.render();
      const wada = result.wadaCandidate
        ? 'Wada candidate ✓'
        : `Wada fraction ${(result.wadaFraction * 100).toFixed(0)}%`;
      this.dom.setText(
        'basinStatus',
        `done · Sb=${result.basinEntropy.toFixed(3)}±${result.basinEntropyStdError.toFixed(3)} · dim=${result.boxCountingDimension.toFixed(3)}±${result.boxCountingStdError.toFixed(3)} (R²=${result.boxCountingR2.toFixed(3)}) · ${wada}`
      );
      this.badge(
        'basinStatus',
        'finite-time-estimate',
        'Basin entropy/dimension: finite-resolution estimates with std errors.',
        {
  ...
  private async runGpu(spec: Extract<SystemSpec, { kind: 'double' }>, n: number): Promise<void> {
    try {
      const field = await flipBasinField({ m1: spec.m1, m2: spec.m2, l1: spec.l1, l2: spec.l2, g: spec.g }, { n });
      ...
      this.dom.setText(
        'basinSbb',
        `${entropy.boundaryBasinEntropy.toFixed(4)} ± ${entropy.boundaryBasinEntropyStdError.toFixed(4)}`
      );
      ...
      this.render();
      const wadaText = wada.wadaCandidate
        ? 'Wada candidate ✓'
        : `Wada fraction ${(wada.wadaFraction * 100).toFixed(0)}%`;
      const backendNote =
        field.backend === 'webgpu'
      ...
      this.dom.setText(
        'basinStatus',
        `done · Sb=${entropy.basinEntropy.toFixed(3)}±${entropy.basinEntropyStdError.toFixed(3)} · dim=${box.dimension.toFixed(3)}±${box.stdError.toFixed(3)} (R²=${box.r2.toFixed(3)}) · ${wadaText} · ${backendNote}`
      );
      this.badge(
        'basinStatus',
        field.backend === 'webgpu'
        ...
        field.caveat,
        {
          ...
          parameters: {
            ...
            validationPassed: field.validation?.passed ?? 'cpu-fallback'
          },
          uncertainty: `CPU probe disagreement fraction ${field.validation?.maxAbsDiff ?? 0}; box-counting SE ${box.stdError.toPrecision(4)}.`,
          externalValidation:
            'GPU field output is accepted only after CPU probe validation, otherwise the CPU f64 grid is returned.',
      ...
      this.dom.setText(
        'basinStatus',
        `WebGPU basin failed: ${err instanceof Error ? err.message : String(err)} — use the CPU path`
      );
    } finally {
      this.running = false;
    }
