...
.theory-compare-controls{display:flex;flex-wrap:wrap;align-items:end;gap:10px}.theory-compare-field{display:grid;gap:5px;min-width:130px;font:600 10px/1.3 var(--font-mono,monospace);color:var(--workbench-text-muted,#737e92)}.theory-compare-field input{width:100%;min-height:38px;padding:7px 9px;border:1px solid var(--workbench-border,rgba(205,214,245,.12));border-radius:8px;background:var(--workbench-raised,#0b0e17);color:var(--workbench-text,#f1f3f8);font:12px/1.3 var(--font-mono,monospace)}.theory-compare-run{min-height:38px;padding:7px 13px;border:1px solid color-mix(in srgb,var(--workbench-live,#72d6e5) 48%,transparent);border-radius:8px;background:color-mix(in srgb,var(--workbench-live,#72d6e5) 12%,var(--workbench-raised,#0b0e17));color:var(--workbench-text,#f1f3f8);font-weight:650}.theory-compare-run:disabled{opacity:.58;cursor:wait}
...
@media(max-width:760px){.theory-outline-list{grid-template-columns:repeat(2,minmax(0,1fr))}.theory-links{grid-template-columns:1fr}.theory-section-body{padding-left:16px}.theory-compare-metrics{grid-template-columns:repeat(2,minmax(0,1fr))}}
...
@media(forced-colors:active){.theory-hero,.theory-outline,.theory-section,.theory-equation,.theory-caveat,.theory-link{forced-color-adjust:auto;border-color:CanvasText;background:Canvas;color:CanvasText;box-shadow:none}}
`;
}
...
export class TheoryTab extends TabController {
  ...
  private comparisonElement(document: Document, locale: TheoryLocale): HTMLElement {
    ...
    controls.append(dtLabel, horizonLabel, run);
    ...
    setText(
      note,
      locale === 'ko'
        ? '해석 범위: 짧은 구간의 표현 일관성 점검이며 독립 검증이 아닙니다. γ>0이면 두 에너지 변화량에 물리적 감쇠 손실이 포함됩니다.'
        : 'Scope: a short-horizon representation-consistency check, not independent validation. With γ>0, both energy-change values include physical dissipation.'
    );
    ...
    run.addEventListener('click', () => {
      ...
      document.defaultView?.requestAnimationFrame(() => {
        try {
          ...
          this.dom.setText(
            'theoryComparePolicy',
            `${result.policy} / ${result.steps.toLocaleString()} · tol ${result.comparisonTolerance.toExponential(2)}`
          );
        } catch (error) {
    ...
    card.append(head, controls, status, metrics, note);
    return card;
  }
