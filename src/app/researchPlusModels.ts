...
export function buildSynchronizationExploration(
  ...
  if (!Number.isFinite(coupling) || coupling < 0) throw new Error('Synchronization coupling must be finite and non-negative.');
  ...
  for (let i = 0; i < n; i += 1) {
    const q = (i + 0.5) / n;
    frequencies[i] = mode === 'mean-field'
      ? Math.max(-2, Math.min(2, halfWidth * Math.tan(Math.PI * (q - 0.5))))
      : 0;
    phases[i] = mode === 'mean-field'
      ? 2 * Math.PI * q + 0.25 * Math.sin(6 * Math.PI * q)
      : i < n / 2 ? 0 : (i % 2 === 0 ? 0 : Math.PI);
  }
  ...
  return {
    ...
    chimera: chimeraDiagnostics(phases, { radius: Math.max(2, Math.floor(n / 8)), coherentThreshold: 0.9, incoherentThreshold: 0.3 })
  };
}
...
export function drawMagneticBasin(canvas: HTMLCanvasElement, grid: MagneticBasinGrid): void {
  ...
  const colors = [[239, 91, 91], [89, 210, 162], [112, 155, 255]] as const;
  for (let i = 0; i < grid.labels.length; i += 1) {
