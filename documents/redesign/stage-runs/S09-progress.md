...
| CP3 | 통합 검증·보존·검토 보고 | 전체 Vitest, required E+U, inventory/golden/secret, diff | 원격 확인 및 최종 검증 완료 |
| STATUS | 검증 후 완료 필드와 원격 확정 | 별도 status push/remote hash | 최종 status commit으로 제출 |
...
- 구현·검토 문서의 원격 존재를 확인한 뒤 최종 필수 검증이 모두 통과했다. 이 기록과 완료 필드를 담은 별도 status commit의 원격 hash까지 확인해야 S09 완료다. 사용자·과학 전문가 검토 완료를 주장하지 않는다.
...
- CP1 전체 typecheck, plan check, production build 및 public artifact audit 통과. CP1 시점에는 CP2 UI/시각/browser 여정과 전체 회귀가 진행 중이었다.
...
- CP3 `a0ff838b0c630bdf61413005e3c3c16e8ef3cc13` commit/push 성공, ls-remote 같은 hash 확인. 최종 검사 중 fetch에서도 로컬/원격 동일, ahead/behind 0/0이었다. 원격 CP3 후 전체 Vitest 2,480/2,480과 typecheck/scoped lint/build/Learn/inventory/secret 검사가 통과했다. 1,360개 추적 파일의 자격증명 패턴 발견은 0이다.
- 최종 production E2E는 단일 실행 160/160 통과(18.8분), 실패·skipped·flaky·전역 오류 0이다. Chromium 데스크톱 80개와 Pixel5 에뮬레이션 80개이며, 시각 기준 갱신이나 자동 retry 없이 실행했다. Learn/Focus 84개와 기존 core/Lab/shell 76개, keyboard/320px/CSS zoom2/axe 및 기존 app.html 열기를 포함한다. 원본 결과는 `tmp/S09-final-playwright.json`, 전체 단위 결과는 `tmp/S09-final-vitest.json`; SHA-256과 보존 증거는 `S09-verification.json`에 기록했다.
- 완료 필드는 lastCompletedStage 9 / nextStage 10 / completedStages 1–9 / activeStage null로 제출한다. S09 review checklist는 `course-one-focus-ko.md`이며 체크박스와 approvedReviewGates [7]을 유지한다. 다음 유효 요청은 보고를 검토한 뒤의 `10단계 실행해줘.`다. 과학 전문가 검토 0/8, 실제 스크린리더·실기기·다른 브라우저와 기존 보안 경고는 남은 범위다. 복구 기준은 S08 `d21bc5b`; 파괴적 복구 작업은 수행하지 않았다.
