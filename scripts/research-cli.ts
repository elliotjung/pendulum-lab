...
function run(args: CliArgs): unknown {
  ...
  switch (command) {
    ...
    case 'gali': {
      // GALI_k alignment index of the double pendulum with the analytic
      // Jacobian driving the tangent flow (no finite-difference floor).
      const params: PendulumParameters = { m1: spec.m1, m2: spec.m2, l1: spec.l1, l2: spec.l2, g: spec.g };
      const gamma = flagNum(flags, 'damping', 0);
    ...
    case 'lqr': {
      // Upright LQR design report for the actuated double pendulum.
    ...
    case 'chainlqr': {
      // Upright LQR for the fully-actuated planar N-chain (all links inverted).
      ...
      return {
        ...
        ...(flags.has('full') ? { gain: design.riccati.K } : { gain: `[${n}×${2 * n} — rerun with --full]` })
      };
    }
    case 'swingup': {
      // Hybrid energy-pump + LQR-capture swing-up from hanging.
      const controlSpec = {
        parameters: { m1: spec.m1, m2: spec.m2, l1: spec.l1, l2: spec.l2, g: spec.g },
        gamma: flagNum(flags, 'damping', 0),
        dt: flagNum(flags, 'dt', 0.005),
    ...
    case 'ilqr': {
      // iLQR swing-up trajectory optimisation (analytic RK4 derivatives).
      const mode = (flags.get('mode') ?? 'full') as ActuationMode;
    ...
    default:
      return {
        ...
        commands: ['lyapunov', 'spectrum', 'zeroone', 'rqa', 'ftle', 'basin', 'wada', 'wadaconv', 'codim2', 'studypoint', 'orbit', 'continue', 'switch', 'pitchfork', 'transcritical', 'melnikov', 'estimate', 'sde', 'nstorus', 'nsbranch', 'arnold', 'toruslyap', 'nsconv', 'drift', 'gali', 'lqr', 'chainlqr', 'swingup', 'ilqr', 'batch'],
