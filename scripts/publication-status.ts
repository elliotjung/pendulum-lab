...
const npm = await jsonRequest(
  `https://registry.npmjs.org/${encodeURIComponent(packageJson.name)}/${encodeURIComponent(packageJson.version)}`
);
const zenodo = await jsonRequest(
  `https://zenodo.org/api/records?q=${encodeURIComponent('"Pendulum Lab" AND "nonlinear pendulum dynamics"')}&size=25`
);
...
const doi = matchingZenodo
  ? String(matchingZenodo.doi ?? (matchingZenodo.metadata as Record<string, unknown> | undefined)?.doi ?? '')
  : '';
