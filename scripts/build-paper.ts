...
interface Certification {
  ...
  caveats: string[];
  figureArtifacts?: Array<{ id: string; path: string; hash: string; description: string }>;
  artifactCrossReferences?: Array<{ artifact: string; producedBy: string; usedBy: string }>;
...
function fmtMaybe(value: unknown, digits = 8): string {
  return typeof value === 'number' && Number.isFinite(value) ? value.toPrecision(digits) : 'n/a';
}

async function main(): Promise<void> {
  const studyText = await readFile('reports/paper-study.json', 'utf8');
  const certificationText = await readFile('reports/flagship-certification.json', 'utf8');
  const externalText = await readFile('reports/flagship-external-check.json', 'utf8');
  const study = JSON.parse(studyText) as Study;
  const certification = JSON.parse(certificationText) as Certification;
  const external = JSON.parse(externalText) as ExternalCheck;
  ...
  const embeddedFigureHashes = [
    { id: 'paper-figure-1', path: 'paper/index.html#figure-1', hash: hashText(fig1), description: 'Embedded A_c and A_PD amplitude comparison SVG.' },
    { id: 'paper-figure-2', path: 'paper/index.html#figure-2', hash: hashText(fig2), description: 'Embedded A_PD/A_c ratio and crossing SVG.' },
    { id: 'paper-figure-3', path: 'paper/index.html#figure-3', hash: hashText(fig3), description: 'Embedded gamma=0.5 strobe bifurcation SVG.' }
  ];
  const certificationFigureArtifacts = certification.figureArtifacts ?? [
    {
      id: 'certified-gap-map-svg',
      path: 'reports/flagship-figure1.svg',
      hash: certification.figureHash,
      description: 'Certified gap-map SVG generated by npm run flagship:certify.'
    }
  ];
  const sourceArtifactLedger = [
    { artifact: 'reports/paper-study.json', command: 'npm run paper:study', hash: hashText(studyText), crossReference: 'Figures 1-3, main table, Appendix A' },
    { artifact: 'reports/flagship-certification.json', command: 'npm run flagship:certify', hash: hashText(certificationText), crossReference: 'Appendix A, Appendix C, reviewer dashboard' },
    { artifact: 'reports/flagship-external-check.json', command: 'npm run flagship:external', hash: hashText(externalText), crossReference: 'Appendix B external A_PD search trace' },
    ...certificationFigureArtifacts.map((artifact) => ({
      artifact: artifact.path,
      command: 'npm run flagship:certify',
      hash: artifact.hash,
      crossReference: artifact.description
    })),
    { artifact: 'paper/index.html', command: 'npm run paper:build', hash: 'generated after Appendix C render', crossReference: 'This manuscript' },
    { artifact: 'paper/paper.pdf', command: 'npm run paper:build', hash: 'generated after Appendix C render', crossReference: 'Print/PDF manuscript' }
  ];
  ...
  const externalTraceRows = external.apdChecks.flatMap((row) => (row.searchTrace ?? []).map((trace) => `<tr><td>${row.gamma.toFixed(2)}</td><td>${esc(trace.stage ?? 'n/a')}</td><td>${fmtMaybe(trace.lo)}</td><td>${fmtMaybe(trace.hi)}</td><td>${fmtMaybe(trace.mid)}</td><td>${fmtMaybe(trace.rhoLow)}</td><td>${fmtMaybe(trace.rhoMid)}</td><td>${fmtMaybe(trace.rhoHigh)}</td></tr>`)).join('\n');
  const embeddedFigureRows = embeddedFigureHashes.map((row) => `<tr><td>${row.id}</td><td>${row.path}</td><td>${row.hash}</td><td>${esc(row.description)}</td></tr>`).join('\n');
  const certificationFigureRows = certificationFigureArtifacts.map((row) => `<tr><td>${row.id}</td><td>${row.path}</td><td>${row.hash}</td><td>${esc(row.description)}</td></tr>`).join('\n');
  const artifactRows = sourceArtifactLedger.map((row) => `<tr><td>${row.artifact}</td><td>${row.command}</td><td>${row.hash}</td><td>${esc(row.crossReference)}</td></tr>`).join('\n');
  const crossReferenceRows = (certification.artifactCrossReferences ?? []).map((row) => `<tr><td>${row.artifact}</td><td>${row.producedBy}</td><td>${esc(row.usedBy)}</td></tr>`).join('\n');
  const reproductionCommands = certification.reproductionCommands ?? ['npm run paper:study', 'npm run flagship:certify', 'npm run flagship:external', 'npm run paper:build', 'npm run reviewer:kit'];
...
<p>${esc(certification.reviewerAppendixNote)} The ratio crossing is localized to γ ∈ [${certification.crossing.lower.toFixed(8)}, ${certification.crossing.upper.toFixed(8)}], with point estimate ${certification.crossing.gamma.toFixed(8)}. The certified gap-map SVG hash is <code>${certification.figureHash}</code>.</p>
<p><strong>Claim boundary.</strong> ${esc(certification.claimBoundary ?? 'The claim is limited to the specified driven-pendulum branch and exported caveat map.')}</p>
<table>
...
<h3>External A<sub>PD</sub> Search Trace</h3>
<p>The search trace below records the external script's bracket expansion and bisection states for the Floquet multiplier crossing ρ = −1. Full rows are in <code>reports/flagship-external-check.json</code>.</p>
<table>
<thead><tr><th>γ</th><th>stage</th><th>lo</th><th>hi</th><th>mid</th><th>ρ(lo)</th><th>ρ(mid)</th><th>ρ(hi)</th></tr></thead>
<tbody>${externalTraceRows}</tbody>
</table>
...
<p>Reproduction command sequence: <code>${reproductionCommands.join(' && ')}</code>.</p>
<h3>Figure hashes</h3>
<table class="artifact-ledger">
<thead><tr><th>Figure</th><th>Path</th><th>Hash</th><th>Description</th></tr></thead>
<tbody>${embeddedFigureRows}${certificationFigureRows}</tbody>
</table>
<table class="artifact-ledger">
<thead><tr><th>Artifact</th><th>Reproduce</th><th>Hash / status</th><th>Cross-reference</th></tr></thead>
<tbody>${artifactRows}</tbody>
</table>
<h3>Artifact cross-references</h3>
<table class="artifact-ledger">
<thead><tr><th>Artifact</th><th>Produced by</th><th>Used by</th></tr></thead>
<tbody>${crossReferenceRows}</tbody>
</table>
