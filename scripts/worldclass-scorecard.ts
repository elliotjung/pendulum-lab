...
const vitest = await readJson<{ numTotalTests?: number; numPassedTests?: number; testResults?: unknown[] }>(
  'reports/vitest-results.json',
  {}
);
const benchmark = await readJson<{ mode?: string; comparison?: { deltas?: unknown[] } | null }>(
  'reports/benchmark-report.json',
  {}
);
const gpuScaleValidation = await readJson<{
  cpuReference?: { ensemble?: { f32ReductionOracle?: { passed?: boolean } } };
}>('reports/gpu-scale-validation.json', {});
...
const gpuAdapterMatrix = await readJson<{ status?: string; coverage?: { passed?: number; required?: number } }>(
...
const mutationAggregateFallback = await readJson<MutationAggregateReport>(
  'reports/mutation/mutation-aggregate.json',
  {}
);
const mutationAggregate = await readJson<MutationAggregateReport>(
  'reports/mutation-aggregate.json',
  mutationAggregateFallback
);
const unitTestSummary =
  Number.isInteger(vitest.numTotalTests) && Array.isArray(vitest.testResults)
...
const has = {
  ...
  mutationAggregateReport:
    (await exists('reports/mutation-aggregate.json')) || (await exists('reports/mutation/mutation-aggregate.json')),
  ...
  gpuAdapterMatrixPass:
    gpuAdapterMatrix.status === 'pass' && gpuAdapterMatrix.coverage?.passed === gpuAdapterMatrix.coverage?.required,
  webgpuHardwarePass: webgpuHardwareValidation.status === 'pass' && webgpuHardwareValidation.backend === 'webgpu',
  webgpuFullSpectrumPass:
    webgpuHardwareValidation.status === 'pass' &&
  ...
  gpuBenchmarkLadderPass:
    gpuBenchmarkLadder.status === 'pass' &&
  ...
  trustInspectorUi:
    resultBadgesSource.includes('openTrustInspector') && resultBadgesSource.includes('trust-inspector-panel'),
  trustInspectorE2e: await exists('e2e/trust-inspector.spec.ts'),
  researchWorkspaceCard:
    researchWorkbenchSource.includes('researchWorkspaceCard') && researchWorkbenchSource.includes('rwWorkspaceSelect'),
  researchWorkspaceList:
    researchWorkbenchSource.includes('workspaces') && storageSyncSource.includes('sanitizeWorkspaceList'),
  researchProjectSessions:
    researchWorkbenchSource.includes('rwProjectName') &&
    researchWorkbenchSource.includes('activeResearchSession') &&
    researchSessionStorageSource.includes('sanitizeResearchSession'),
  ...
  ciRunsGpuScale:
    ciWorkflow.includes('npm run validate:gpu-scale') || mainWorkflow.includes('npm run validate:gpu-scale'),
  ...
  chaosAccelerationContracts:
    accelerationContractSource.includes('compareClvAcceleration') &&
    accelerationContractSource.includes('compareFtleFieldAcceleration') &&
    accelerationContractSource.includes('compareLyapunovSpectrumAcceleration'),
  fullSpectrumGpuPromotion:
    gpuLyapunovSource.includes('promotedDoublePendulumLyapunovSpectrum') &&
    gpuLyapunovSource.includes('webgpuDoublePendulumLyapunovSpectrumCandidate'),
  clvFtleGpuPromotion:
    gpuChaosPromotionSource.includes('promotedDoublePendulumClv') &&
    gpuChaosPromotionSource.includes('promotedDoublePendulumVariationalFtleField'),
  nChainGpuPromotion:
    gpuNChainSource.includes('promotedNChainVariational') && gpuNChainSource.includes('WGSL_NCHAIN_VARIATIONAL_KERNEL'),
  npmOidcPublishing:
    npmWorkflow.includes('id-token: write') &&
    npmWorkflow.includes('npm@11.5.1') &&
    npmWorkflow.includes('npm publish --access public'),
  slsaAttestation:
  ...
  attestationsVerified:
    attestationMatchesCurrentCoordinate &&
...
const releasePackageReady =
  has.releasePackageCommand &&
  has.releaseReadiness &&
  has.releaseOnePagePdf &&
  has.walkthroughGif &&
  has.mainRunsReleasePackage &&
  has.releaseReadyStatus;
...
const benchmarkReady =
  has.benchmark && has.energy && (benchmarkHasComparison || benchmarkIsHonestProfile) && has.gpuBenchmarkLadderPass;
const flagshipReady =
  has.certifiedWorkbenchModule && has.flagshipDoc && certifiedWorkbenchSource.includes('melnikov-gap-map');
const flagshipCertified =
  flagshipReady &&
  has.flagshipCertifyCommand &&
  has.flagshipCertification &&
  has.flagshipFigure &&
  has.flagshipExternalCommand &&
  has.flagshipExternalCheck;
const reviewerKitReady =
  flagshipCertified &&
  has.reviewerKitDoc &&
  has.reviewerKitScript &&
  has.reviewerKitCommand &&
  has.reviewerKitManifest &&
  has.reviewerKitManifestMd;
const gpuScaleReady =
  has.gpuScaleCommand &&
  has.gpuScaleScript &&
  has.gpuScaleReport &&
  has.gpuScaleJson &&
  has.gpuReductionOracle &&
  has.ciRunsGpuScale &&
  has.webgpuHardwareWorkflow &&
  has.webgpuHardwareE2e &&
  has.webgpuHardwareCommand &&
  has.webgpuHardwareValidateCommand &&
  has.gpuBenchmarkLadderCommand &&
  has.gpuAdapterMatrixCommand &&
  has.webgpuWorkflowRunsValidation &&
  has.webgpuWorkflowRunsBenchmarkLadder &&
  has.webgpuWorkflowRunsAdapterMatrix &&
  has.webgpuHardwareReport &&
  has.webgpuHardwareJson &&
  has.gpuBenchmarkLadderReport &&
  has.gpuBenchmarkLadderJson &&
  has.gpuAdapterMatrixReport &&
  has.gpuAdapterMatrixJson &&
  has.webgpuHardwarePass &&
  has.webgpuFullSpectrumPass &&
  has.webgpuClvPass &&
  has.webgpuVariationalFtlePass &&
  has.webgpuNChainPass &&
  has.gpuBenchmarkLadderPass;
const chaosAccelerationReady =
  has.chaosAccelerationContracts && has.fullSpectrumGpuPromotion && has.clvFtleGpuPromotion && has.nChainGpuPromotion;
const externalPublicationReady =
  has.npmOidcPublishing &&
  has.slsaAttestation &&
  has.attestationsVerified &&
  has.publicationStatusReport &&
  has.npmPublished &&
  has.zenodoPublished &&
  has.githubReleasePublished &&
  has.pagesPublished;
const sparseFloquetReady = has.arnoldiSchurFloquet;
const trustWorkspaceReady =
  has.trustInspectorUi &&
  has.trustInspectorE2e &&
  has.researchWorkspaceCard &&
  has.researchWorkspaceList &&
  has.researchProjectSessions;
...
const items: ScorecardItem[] = [
  ...
  },
  {
    ...
    evidence: [
      'app.html is the source shell; Pages emits dist/index.html and releases emit a SHA-256-pinned standalone/index.html without committing generated root blobs'
    ],
  ...
  },
  {
    ...
    evidence: [
      flagshipReady
        ? 'flagship module and docs name the Melnikov threshold vs period-doubling gap map as the crown result'
        : 'flagship result contract missing',
      has.flagshipCertification
        ? 'flagship certification report exists with Figure 1 hash, crossing interval, onset table, and caveat map'
        : 'flagship certification report missing',
      has.flagshipExternalCheck
        ? 'dependency-free Python external check exists for A_c and ratio crossing arithmetic'
        : 'flagship external check missing',
    ...
    remaining: [
      ...
      ...(!flagshipCertified
        ? ['Run npm run flagship:certify and npm run flagship:external to generate Figure 1 certification artifacts']
        : []),
  ...
  },
  {
    ...
    evidence: [
      has.trustInspectorUi
        ? 'result badges open a Trust Inspector with source, parameters, uncertainty, external validation, reproduce command, caveat, artifact, and hash fields'
        : 'Trust Inspector badge panel missing',
      has.trustInspectorE2e
        ? 'Trust Inspector DOM/e2e spec verifies click, keyboard open, content, Escape, and close behavior'
        : 'Trust Inspector DOM/e2e spec missing',
      has.researchWorkspaceCard
        ? 'Research tab includes the Certified Workspace card'
        : 'Certified Workspace card missing',
      has.researchWorkspaceList
        ? 'workspace profile list and storage sanitizer exist'
        : 'workspace profile list or storage sanitizer missing',
      has.researchProjectSessions
        ? 'workspace storage now models Project -> Sessions -> Runs -> Artifacts'
        : 'project/session/artifact hierarchy missing',
      'audience modes gate beginner/student/research surfaces'
    ],
    remaining: trustWorkspaceReady
      ? []
      : ['Complete Trust Inspector e2e coverage and Project -> Sessions -> Runs -> Artifacts workspace hierarchy']
  },
  {
    ...
    evidence: [
      ...
      gpuScaleReady
        ? 'GPU/scale validation contract report exists, CI runs it, and real WebGPU hardware validation has passed against the CPU oracle'
        : 'GPU/scale validation contract or CI wiring is incomplete',
      sparseFloquetReady
        ? 'matrix-free Arnoldi-Schur Floquet wrapper exists for sparse/large unitary operators'
        : 'sparse/large Floquet Arnoldi-Schur wrapper missing'
    ],
  ...
  },
  {
    ...
    evidence: [
      ...
      has.gpuScaleReport
        ? 'gpu-scale validation report separates acceleration caveats from scientific claims'
        : 'gpu-scale validation report missing',
      chaosAccelerationReady
        ? 'CLV, full-spectrum, and FTLE acceleration promotion contracts are executable comparison functions'
        : 'CLV/FTLE/full-spectrum acceleration contracts missing',
      has.webgpuFullSpectrumPass
        ? '4D double-pendulum full-spectrum WebGPU candidate passed the hardware CPU-oracle promotion gate'
        : 'full-spectrum WebGPU hardware promotion evidence missing',
      has.webgpuClvPass
        ? '4D double-pendulum CLV WebGPU candidate passed the hardware CPU-oracle promotion gate'
        : 'CLV WebGPU hardware promotion evidence missing',
      has.webgpuVariationalFtlePass
        ? '4D double-pendulum variational-FTLE WebGPU candidate passed the hardware CPU-oracle promotion gate'
        : 'variational-FTLE WebGPU hardware promotion evidence missing',
      has.webgpuNChainPass
        ? '6D planar N-chain tiled STM/QR/CLV/FTLE candidate passed the hardware CPU-f64 oracle gate'
        : 'N-chain WebGPU hardware promotion evidence missing'
    ],
    remaining:
      chaosAccelerationReady &&
      has.webgpuFullSpectrumPass &&
      ...
      has.webgpuNChainPass
        ? []
        : ['Add or run hardware-gated CLV/FTLE/full-spectrum/N-chain acceleration comparison contracts']
  },
  {
    ...
    evidence: [
      ...
      has.gpuReductionOracle
        ? 'ensemble f32-candidate reduction passes the CPU f64 oracle comparison'
        : 'ensemble reduction oracle result missing from gpu-scale validation JSON',
      has.ciRunsGpuScale ? 'CI runs validate:gpu-scale' : 'CI does not run validate:gpu-scale',
      has.webgpuHardwareWorkflow
        ? 'self-hosted WebGPU hardware workflow exists and fails when a real adapter is absent'
        : 'WebGPU hardware workflow missing',
      has.webgpuHardwareE2e
        ? 'hardware e2e compares GPU-side ensemble reduction, full-spectrum, CLV, variational-FTLE, and N-chain STM/QR promotion with CPU f64'
        : 'hardware GPU reduction e2e missing',
      has.webgpuHardwareValidateCommand
        ? 'npm run validate:webgpu-hardware exists'
        : 'validate:webgpu-hardware command missing',
      has.gpuBenchmarkLadderCommand ? 'npm run benchmark:gpu-ladder exists' : 'GPU benchmark ladder command missing',
      has.webgpuWorkflowRunsValidation
        ? 'self-hosted WebGPU workflow writes the hardware validation report'
        : 'WebGPU workflow does not run validate:webgpu-hardware',
      has.webgpuWorkflowRunsBenchmarkLadder
        ? 'self-hosted WebGPU workflow writes the GPU benchmark ladder report'
        : 'WebGPU workflow does not run benchmark:gpu-ladder',
      has.webgpuHardwarePass
        ? 'reports/webgpu-hardware-validation.json records backend=webgpu and status=pass'
        : 'hardware WebGPU validation report is missing or not passing',
      has.webgpuFullSpectrumPass
        ? 'hardware WebGPU validation report records full-spectrum backend=webgpu and comparison pass'
        : 'hardware full-spectrum promotion evidence missing',
      has.webgpuClvPass
        ? 'hardware WebGPU validation report records CLV backend=webgpu and comparison pass'
        : 'hardware CLV promotion evidence missing',
      has.webgpuVariationalFtlePass
        ? 'hardware WebGPU validation report records variational-FTLE backend=webgpu and comparison pass'
        : 'hardware variational-FTLE promotion evidence missing',
      has.webgpuNChainPass
        ? 'hardware WebGPU validation report records N-chain STM/QR backend=webgpu and comparison pass'
        : 'hardware N-chain promotion evidence missing',
      has.gpuBenchmarkLadderReport
        ? 'reports/gpu-benchmark-ladder.md records adapter metadata, horizon drift, and promotion metrics'
        : 'GPU benchmark ladder report missing',
      has.gpuBenchmarkLadderPass
        ? 'reports/gpu-benchmark-ladder.json records status=pass across reductions, 4D diagnostics, and N-chain STM/QR gates'
        : 'GPU benchmark ladder pass evidence missing',
      has.gpuAdapterMatrixReport
        ? `multi-adapter matrix records ${gpuAdapterMatrix.coverage?.passed ?? 0}/${gpuAdapterMatrix.coverage?.required ?? 3} Intel/NVIDIA/AMD vendor classes`
        : 'multi-adapter matrix report missing',
    ...
    remaining: [
      ...(gpuScaleReady
        ? []
        : [
            'Run npm run validate:gpu-scale, npm run validate:webgpu-hardware, npm run benchmark:gpu-ladder, and npm run benchmark:gpu-matrix'
          ]),
      ...(has.gpuAdapterMatrixPass
        ? []
        : ['Register physical NVIDIA and AMD self-hosted WebGPU runners and collect passing vendor ladder artifacts'])
    ]
  },
  {
    ...
    evidence: [
      ...
      testTierReady
        ? 'quick, slow, and full test tiers are wired into PR/mainline workflows'
        : 'quick/slow/full test tier wiring is incomplete',
      has.coverageScopeBaseline
        ? 'coverage scope guard catches new source files missing from the v8 coverage map'
        : 'coverage scope guard missing',
      has.longRunE2e
        ? 'long-run performance/soak e2e spec exists and runs in mainline full validation'
        : 'long-run performance/soak e2e spec missing',
      has.accessibilityE2e
        ? 'accessibility e2e spec exists and runs in mainline full validation'
        : 'accessibility e2e spec missing',
      visualReady
        ? 'visual regression script, spec, and versioned Chromium snapshots exist'
        : 'visual regression command or snapshots are missing',
      memoryReady
        ? 'memory-regression report and baseline exist from benchmark output'
        : 'memory-regression report or baseline missing',
      mutationReady
        ? `nightly mutation aggregate passes at ${mutationAggregate.mutationScore}% total / ${mutationAggregate.coveredMutationScore}% covered across ${mutationAggregate.reportCount} shards`
        : 'nightly mutation aggregate missing or below threshold'
    ],
    remaining: [
      ...(!testTierReady ? ['Wire quick/slow/full test tiers into CI'] : []),
      ...(!visualReady ? ['Promote visual regression command and golden snapshots'] : []),
      ...(!memoryReady
        ? ['Run npm run benchmark and npm run benchmark:memory to create memory-regression report + baseline artifacts']
        : []),
      ...(!mutationReady
        ? ['Run the Nightly Mutation workflow and publish reports/mutation-aggregate.json before a release review']
        : [])
    ]
  },
  {
    ...
    evidence: [
      ...
      has.gpuBenchmarkLadderPass
        ? 'gpu-benchmark-ladder records WebGPU adapter metadata, f32/f64 horizon drift, and CPU-oracle promotion metrics'
        : 'GPU benchmark ladder missing or not passing',
      has.bundleBudget
        ? 'bundle budget gate splits initial/chunk/standalone assets across raw/gzip/brotli sizes'
        : 'bundle budget gate missing',
      has.mainRunsBenchmark
        ? 'mainline workflow runs the browser benchmark'
        : 'mainline workflow does not run the browser benchmark',
      has.mainRunsMemoryRegression
        ? 'mainline workflow emits memory-regression artifacts'
        : 'mainline workflow does not emit memory-regression artifacts'
    ],
    remaining: [
      ...(!benchmarkReady ? ['Run npm run benchmark and benchmark:energy after performance-affecting changes'] : []),
      ...(!has.mainRunsBenchmark || !has.mainRunsMemoryRegression
        ? ['Wire benchmark and memory-regression scripts into mainline CI']
        : []),
  ...
  },
  {
    ...
    evidence: [
      ...
      has.mainRunsMojibakeStrict
        ? 'mainline workflow runs strict mojibake audit'
        : 'mainline workflow does not run strict mojibake audit'
    ],
    remaining: legacyClean
      ? []
      : [
          `innerHTML=${legacy.counts.innerHTML}`,
          `onclick=${legacy.counts.onclick}`,
          `inlineWorkerBlob=${legacy.counts.inlineWorkerBlob}`,
          `dynamicScript=${legacy.counts.dynamicScript}`,
          `globalRuntimeExports=${legacy.counts.globalRuntimeExports}`
        ]
  },
  {
    ...
    evidence: [
      has.npmOidcPublishing
        ? 'npm workflow uses tokenless OIDC trusted publishing with an exact-version guard'
        : 'npm OIDC trusted publishing workflow missing',
      has.slsaAttestation
        ? 'release workflow emits SLSA/in-toto build provenance and CycloneDX SBOM attestations with actions/attest@v4'
      ...
      has.zenodoPublished
        ? `Zenodo DOI ${publicationStatus.zenodo?.doi} resolves publicly`
        : 'public Zenodo DOI is not minted'
    ],
    remaining: [
      ...(!has.npmPublished
        ? [
            'Configure npm trusted publisher for publish-npm.yml/environment npm or supply an owner token for the first publish'
          ]
        : []),
      ...
      ...(!has.attestationsVerified
        ? ['Run npm run release:verify-attestations against the published release tarball']
        : [])
    ]
  },
  {
    ...
    evidence: [
      ...
      has.reviewerDashboardE2e
        ? 'reviewer dashboard evidence dialog, tabs, and ledger have browser coverage'
        : 'reviewer dashboard e2e missing',
      has.mainWorkflow ? 'mainline full-validation workflow exists' : 'mainline full-validation workflow missing',
      mutationReady
        ? 'nightly mutation workflow and aggregate report exist'
        : has.nightlyWorkflow
          ? 'nightly mutation workflow exists; aggregate report missing or below threshold'
          : 'nightly mutation workflow missing',
      has.releaseWorkflow ? 'release artifact workflow exists' : 'release artifact workflow missing',
      has.distIndex
        ? 'dist/index.html exists for Pages artifact deployment'
    ...
    remaining: [
      ...
      ...(externalPublicationReady
        ? []
        : ['External npm publication and Zenodo DOI are tracked separately in reports/publication-status.json'])
    ]
  }
];
...
function markdown(): string {
  ...
  for (const item of items) {
    lines.push(
      `| ${item.area} | ${item.status.toUpperCase()} | ${item.evidence.join('<br>')} | ${item.remaining.join('<br>')} |`
    );
  }
  return `${lines.join('\n')}\n`;
}
